...
CUDA_PATH=${CUDA_PATH:-/scratch/opt/cuda-12.1/}
if [ ! -d $CUDA_PATH ]
then
...
echo "make PREFIX=${install_path}/redshow BOOST_DIR=$B GPU_PATCH_DIR=${install_path}/gpu-patch DEBUG=1  STATIC_CPP=1 install -f Makefile.static"
make PREFIX=${install_path}/redshow BOOST_DIR=$B GPU_PATCH_DIR=${install_path}/gpu-patch DEBUG=1  STATIC_CPP=1 install -f Makefile.static
